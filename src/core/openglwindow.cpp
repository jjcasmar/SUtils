...
OpenGLWindow::OpenGLWindow(bool /*offScreenRendering*/)
    : Qt3DWindow(), m_rootEntity(new Qt3DCore::QEntity),
...
//    m_trackballCameraController(new
//    Qt3DExtras::QTrackballCameraController(m_internalRootEntity))
{
  ...
  Qt3DRender::QClearBuffers *backgroundClearBuffers =
      new Qt3DRender::QClearBuffers(framegraph);
  ...
  Qt3DRender::QCameraSelector *backgroundCameraSelector =
      new Qt3DRender::QCameraSelector(backgroundClearBuffers);
  Qt3DRender::QLayerFilter *layerFilter =
      new Qt3DRender::QLayerFilter(backgroundCameraSelector);
  ...
  // Qt3DRender::QRenderTargetSelector(viewport);
  Qt3DRender::QRenderSurfaceSelector *renderSurfaceSelector =
      new Qt3DRender::QRenderSurfaceSelector(viewport);
  ...
  Qt3DRender::QClearBuffers *forwardDefaultClearBuffers =
      forwardRenderer->findChild<Qt3DRender::QClearBuffers *>();
  forwardDefaultClearBuffers->setBuffers(
  ...
  Qt3DRender::QCamera *backgroundCamera =
      new Qt3DRender::QCamera(backgroundCameraSelector);
  ...
  Qt3DCore::QEntity *backgroundEntity =
      new Qt3DCore::QEntity(m_internalRootEntity);
  Qt3DExtras::QPlaneMesh *backgroundMesh =
      new Qt3DExtras::QPlaneMesh(backgroundEntity);
  Qt3DExtras::QPerVertexColorMaterial *backgroundMaterial =
      new Qt3DExtras::QPerVertexColorMaterial(backgroundEntity);
  Qt3DCore::QTransform *backgroundTransform =
      new Qt3DCore::QTransform(backgroundEntity);
  backgroundTransform->setRotation(
  ...
  dynamic_cast<Qt3DExtras::QPlaneGeometry *>(floorGeometryRenderer->geometry())
      ->setResolution(QSize(20, 20));
  m_floorGeometry = dynamic_cast<Qt3DExtras::QPlaneGeometry *>(
      floorGeometryRenderer->geometry());
  ...
  Qt3DRender::QClearBuffers *gnomonClearBuffer =
      new Qt3DRender::QClearBuffers(gnomonViewport);
  Qt3DRender::QCameraSelector *gnomonCameraSelector =
      new Qt3DRender::QCameraSelector(gnomonClearBuffer);
  //    Qt3DRender::QLayerFilter *gnomonLayerFilter = new
  //    Qt3DRender::QLayerFilter(gnomonCameraSelector);
  ...
  gnomonGeometry->setSource(
      QUrl::fromLocalFile("qrc:/geometry/gnomon/gnomon.gltf"));
  Qt3DExtras::QPerVertexColorMaterial *colorMaterial =
...
void OpenGLWindow::takeImage() {
  Qt3DRender::QRenderCaptureReply *captureReply =
      m_renderCaptureFrameGraph->requestCapture();
  QObject::connect(captureReply, &Qt3DRender::QRenderCaptureReply::completed,
                   [this, captureReply]() {
                     captureReply->saveImage(this->m_imageFilename);
                     captureReply->deleteLater();
                     this->imageTaken();
                   });
}

QString OpenGLWindow::imageFilename() const { return m_imageFilename; }

void OpenGLWindow::setImageFilename(const QString &imageFilename) {
  m_imageFilename = imageFilename;
}

Qt3DCore::QEntity *OpenGLWindow::rootEntity() const { return m_rootEntity; }

Qt3DExtras::QTrackballCameraController *
OpenGLWindow::trackballCameraController() const {
...
void OpenGLWindow::resizeEvent(QResizeEvent *ev) {
  ...
  this->camera()->setAspectRatio(float(ev->size().width()) /
